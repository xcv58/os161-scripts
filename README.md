...
```bash
wget -qO- http://os161.xcv58.com | sudo bash
```
...
```
build_run_os161.sh br
```
The script will run the instruction for DUMBVM:
```
bmake depend
bmake
bmake install
...
### Run
The first argument should be b, r, rw, br, brw, bv, bvr, bvrw, or d.

"r" means directly run kernel.

"rw" means run kernel in debug mode.
...
For example:
```
build_run_os161.sh brw /testbin/parallelvm
```
